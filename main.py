...
class InstrScr(Screen):
    def __init__(self, **kw):
        ...
        lbl2 = Label(text="Введите ваш возраст: ", halign='right')
...
class PulseScr(Screen):
    def __init__(self, **kw):
        ...
        lbl_result = Label(text='Введите результат: ', multiline=False)
        self.in_result.setdisabled(True)
        
        line.add_widget(lbl_result)
        line.add_widget(self.in_result)
        self.btn = Button(text='Начать', size_hint=(0.3, 0.4), pos_hint={'center_x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout(orintation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
...
class CheckSits(Screen):
    def __init__(self, **kw):
        ...
        self.btn.on_press = self.next
        outer = BoxLayout(orintation='vertical', pading=8, spacing=8)
        outer.add_widget(instr)
        outer.add_widget(self.btn)
        self.add_widget(outer)
...
class PulseScr2(Screen):
    def __init__(self, **kw):
        ...
        self.add_widget(lbl_result2)
        self.btn = Button(text='Начать', size_hint=(0.3, 0.5), pos_hint={'senter_x': 0.5})
    ...
    def next(self):
        if not self.next_screen:
            ...
            p3 = check_int(self.in_result1.text)
            if p2 == False:
                p2 = 0
                self.in_result1 = str(p2)
            elif p3 == False:
                p3 = 0
...
class Result(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.outer = BoxLayout(orintation='vertical', padding=8, spacing=8)
